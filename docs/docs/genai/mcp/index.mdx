...
The `mlflow mcp run` command described here runs the MCP server locally over stdio. A tracking server can also serve the same tools over HTTP, so a shared server is reachable from any MCP client without a local MLflow install. See [Remote MCP Server](/genai/mcp/remote-server).
...
- [Remote MCP Server](/genai/mcp/remote-server)
