...
| Serve MLflow tools to AI assistants over MCP | `--enable-mcp`                                      | [Remote MCP Server](/genai/mcp/remote-server)                               |
...
:::warning
The opt-in executor engine currently supports only single-replica MLflow deployments; overlapping
rolling restarts are also unsupported. Multi-replica coordination will be supported after
scheduler leadership and stale-lease recovery are implemented.
:::
